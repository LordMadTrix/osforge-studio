...
- **37. 🎨 & 🖥️ Fond d'Écran Haute Fidélité (Protection XML / SVG) & Session X11 Cinnamon Native sous GDM3 / LightDM** :
  - **Diagnostic & Root Cause (Découvert via la capture d'écran utilisateur de la VM QEMU/VirtualBox)** :
    ...
    - *Barre des tâches / Panel Cinnamon manquant sous GDM3* : L'utilisateur avait configuré Cinnamon avec GDM3 (`displayManager: 'gdm3'`). Sous Debian Trixie, le paquet `cinnamon` déploie à la fois `/usr/share/xsessions/cinnamon.desktop` et `/usr/share/wayland-sessions/cinnamon-wayland.desktop` (intitulé *Cinnamon on Wayland (Experimental)*). GDM3 priorisait la session Wayland expérimentale qui, sous VM (QEMU / VirtualBox) sans 3D virgl complète, n'initialisait pas le panel muffin/clutter, laissant uniquement `nemo-desktop` avec les icônes sur un écran noir sans barre des tâches. De plus, `dmAutologinCmd` ne spécifiait pas la session dans AccountsService (`/var/lib/AccountsService/users/${username}`) ni ne désactivait Wayland pour les DEs X11 traditionnels.
    - *Inclusion dynamique du Display Manager dans `packages.ts`* : `resolvePackageList` n'injectait pas systématiquement le display manager sélectionné (`recipe.displayManager`) s'il différait de la valeur par défaut du bureau.
  - **Résolution Appliquée (Zéro Cosmétique)** :
    - *Protection Universelle XML & SVGs (`branding.ts`)* : Création de `escapeXml()` et `cleanXmlEntities()`. Application systématique sur `osName` et `editionName` dans tous les presets de fond d'écran et logos, assurant que toute entité XML (`&amp;`, `&lt;`, `&gt;`, `&quot;`, `&apos;`) soit 100% valide et reconnue par `rsvg-convert`.
    - *Propriétés Fonds d'Écran Cinnamon, GNOME et Desktop-Base (`branding.ts`)* : Génération automatique des fichiers de métadonnées `/usr/share/cinnamon-background-properties/${slug}.xml`, `/usr/share/gnome-background-properties/${slug}.xml` et duplication de secours dans `/usr/share/images/desktop-base/`.
    - *Session X11 Native & Profils AccountsService (`helpers.ts`)* : Implémentation de `getDesktopSessionName()`. Pour GDM3 avec des bureaux non-Wayland natifs (Cinnamon, XFCE, MATE, LXQt, LXDE), injection automatique de `WaylandEnable=false` dans `/etc/gdm3/custom.conf`, et écriture du profil utilisateur AccountsService `/var/lib/AccountsService/users/${username}` (`Session=cinnamon`, `XSession=cinnamon`). Pour LightDM, inclusion systématique de `user-session=${session}` dans `50-autologin.conf`. Pour SDDM, limitation de `10-wayland.conf` à KDE exclusivement.
    - *Résolution universelle des Display Managers (`packages.ts`)* : Ajout dynamique du paquet approprié (`gdm3`, `lightdm`, `sddm`, `ly`) selon le choix de l'utilisateur.
  - **Résultat** : 6 nouveaux tests unitaires Vitest ajoutés (861 tests au total, 100% au vert).
...
- **Résolution Définitive Écran Blanc Localhost 5173 & Packaging Intégral de Production** :
  ...
  - Correctifs appliqués :
    ...
    6. Élimination totale des caractères corrompus (mojibake `Ã©`, `Ã¨`, `Ã‰`, `Ãª`) dans la console Windows : passage en texte ASCII pur pour toutes les bannières CLI (.bat et .ps1) et injection du BOM UTF-8 (`\uFEFF`) sur les scripts PowerShell afin de garantir un affichage 100% net sous tous les shells Windows (CMD, conhost, Windows Terminal, PowerShell 5.1 et 7).
