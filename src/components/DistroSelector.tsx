...
export const DistroSelector: React.FC<DistroSelectorProps> = ({ recipe, onChange, lang, onOpenTips, onOpenScreenshots }) => {
  ...
  const formats: { id: OutputFormat; name: string; desc: string; icon: string; tooltipFr: string; tooltipEn: string }[] = [
    ...
    {
      id: 'proxmox_qcow2',
      name: 'Template Proxmox VE (.qcow2)',
      desc: 'QCOW2 + Cloud-Init + qemu-guest-agent & script deploy-proxmox.sh',
      icon: '🚀',
      tooltipFr: 'Image optimisée Proxmox VE prête à convertir en template VM avec script d’import 1-clic.',
      tooltipEn: 'Proxmox VE template image with Cloud-Init, guest agent, and 1-click import script.',
    },
    {
      id: 'ami_raw',
      name: 'Image Cloud AWS AMI / OpenStack (RAW)',
      desc: 'Image RAW Sparse pour AWS EC2 / OpenStack Glance',
      icon: '🌐',
      tooltipFr: 'Image disque brute sparse prête pour aws ec2 import-snapshot ou OpenStack.',
      tooltipEn: 'Raw sparse disk image ready for AWS EC2 snapshot import or OpenStack Glance.',
    },
    {
      id: 'vmdk',
      name: 'Disque Virtuel VMware (VMDK)',
      desc: 'Prêt pour VMware Workstation, ESXi / vSphere',
      icon: '💻',
      tooltipFr: 'Format universel pour importer directement dans VMware Workstation et vSphere ESXi.',
      tooltipEn: 'Universal virtual disk format for VMware Workstation and vSphere ESXi.',
    },
    {
      id: 'vdi',
      name: 'Disque Virtuel VirtualBox (VDI)',
      desc: 'Format natif Oracle VirtualBox 7.x',
      icon: '📦',
      tooltipFr: 'Format disque natif VirtualBox pour une exécution directe sans conversion.',
      tooltipEn: 'Native VirtualBox disk format for direct execution without extra conversion.',
    },
    {
      id: 'raw_img',
